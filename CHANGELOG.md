...
- **CI/CD pipeline** — dropped Node 18 from test matrix (vitest v4 requires Node 20+), added `.prettierignore` for `.github/` and `website/` directories.
- **First npm release** — publish to npm via `release.yml` workflow.

## [0.1.2] - 2026-08-19

### Fixed

- **`dist/` now committed** — `.gitignore` previously had a bare `dist` rule that prevented the build output from being tracked. Consumers installing via `npm install github:chaeco/ip-lookup` now receive the published artifacts.
